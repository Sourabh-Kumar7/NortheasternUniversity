...
arrange(df, Private)
arrange(df, desc(Private)) # descending
...
## [your code here]
# this first selects only "X" and "Grad.Rate" columns
# then it arranges the rows by descending Grad.Rate 
# finally it only saves those rows for which Grad.Rate is bigger than 80
df4=filter(arrange(select(df, X, Grad.Rate), desc(Grad.Rate)), Grad.Rate >= 80)
df4
...
## [your code here]
# the idea is the same for both college and df
# first, group the dataset by "Private"
# summarise both groups, forming two new columns max and min
# max column shows the calculated max for a group, min shows the calculated min for a group
dfmm=group_by(df, Private)
dfmm=summarise(dfmm, max=max(Outstate), min=min(Outstate))
dfmm

collegemm=group_by(college, Private)
collegemm=summarise(collegemm, max=max(Outstate), min=min(Outstate))
collegemm
